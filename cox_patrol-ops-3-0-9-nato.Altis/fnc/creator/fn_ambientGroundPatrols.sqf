...
waitUntil {
	sleep 1;
	{
		...
		// _activeVehicleCount = count _activeVehicles;
		// If entity is active
		if(_entity select 1) then {
			...
			if(
				...
				// _activeVehicles = _activeVehicles - [_entity];
				_believedCount = _believedCount - 1;
			...
			_result_2 = { (_entity select 2) distance _x < (_cacheRadius*2)} count _basePosi;
			_result_3 = _activeVehicleCount < _maxVehiclesActive;
			...
			if( count ([_position,_cacheRadius,[west,east,resistance],["CAManBase","LandVehicle","Helicopter"] ] call PO3_fnc_getNearbyPlayers) > 0
				...
				_groupID = if( _type == "INF" ) then {
					...
					if(_believedCount < _maxVehiclesActive) then {
				...
				if(_added) then {
					...
					_believedCount = _believedCount + 1;
					// _activeVehicles = _activeVehicles pushBack _entity;
