	...
	if (({(side _x == side_blue) && (_x distance Stranger < 200)} count allPlayers < 1) && ({(side _x isEqualTo civilian) && (_x distance Stranger < 200)} count allPlayers > 0)) then {
		...
		while {({(side _x == side_blue) && (_x distance Stranger < 200)} count allPlayers < 1) && ({(side _x isEqualTo civilian) && (_x distance Stranger < 200)} count allPlayers > 0) && (dateToNumber date < _fechalimnum)} do {
			...
			if (!(_acc) && {(side _x isEqualTo civilian) && (_x distance Stranger < 5)} count allPlayers > 0) exitWith {
				...
				_contact = true;
				/*
				_mrk = createMarker [format ["Stranger%1", random 1000], _posDealer];
				_mrk setMarkerShape "ICON";
				_mrk setMarkerType "flag_FIA";
				_mrk setMarkerColor "ColorWhite";
				_mrk setMarkerText "Stranger";
				*/
...
/*
while {({(side _x == side_blue) && (_x distance Stranger < 500)} count allPlayers < 1) && ({(side _x isEqualTo civilian) && (_x distance Stranger < 500)} count allPlayers > 0) && (dateToNumber date < _fechalimnum)} do {
	if ({(side _x isEqualTo civilian) && (_x distance Stranger < 2)} count allPlayers > 0) exitWith {
		_tsk = ["FND_C",[side_blue,civilian],[format ["A former associate of Petros' is trying to leave Altis. His departure is scheduled for %2:%3. If you manage to get to him in time, you might be able to convince him to stay and support our case. Unfortunatly, we are unaware of his current location, but he was spotted near %1. Most likely, he'll have taken shelter in a building...",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Find the corrupt politician",_site],_posDealer,"ASSIGNED",5,true,true,"Find"] call BIS_fnc_setTask;
		_acc = true;
		_mrkDealer = createMarker ["Stranger", _posDealer];
		_mrkDealer setMarkerShape "ICON";
		[[Stranger,"misCiv"],"flagaction"] call BIS_fnc_MP;
	};

	if (({(side _x isEqualTo civilian) && (_x distance Stranger < 200)} count allPlayers < 1) && (_acc)) exitWith {};
};

waitUntil {sleep 1; ({(side _x isEqualTo civilian) && (_x distance Stranger < 200)} count allPlayers < 1) || (not alive Stranger) || ({(side _x == side_blue) && (_x distance Stranger < 200)} count allPlayers > 0)};
*/
