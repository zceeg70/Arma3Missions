...
if (_backpack != "") then {
	...
	_switch = true;
/*
	if (_backpack find "I_Carryall_oli" >= 0) then {
		removeBackpack _unit;
		_unit addBackpack "B_Carryall_oli";
		_switch = true;
	};

	if (_backpack find "I_Fieldpack_oli" >= 0) then {
		removeBackpack _unit;
		_unit addBackpack "B_FieldPack_oli";
		_switch = true;
	};

	if ((_backpack find "G_TacticalPack" >= 0) || (_backpack find "G_FieldPack" >= 0) || (_backpack find "G_Carryall" >= 0) || (_backpack find "G_AssaultPack" >= 0)) then {
		removeBackpack _unit;
		_unit addBackpack "B_Carryall_oli";
		_switch = true;
	};
...
_mags = [];
_destino = cajaVeh;
if (([_unit] call isMember) && ((server getVariable "enableMemAcc"))) exitWith {["Open",[nil,_box,_unit]] call bis_fnc_arsenal;};
