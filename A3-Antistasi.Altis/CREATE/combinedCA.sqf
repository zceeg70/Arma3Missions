...
private ["_posorigen","_tipogrupo","_nombreorig","_markTsk","_wp1","_soldados","_landpos","_pad","_vehiculos","_wp0","_wp3","_wp4","_wp2",
		...
		"_tiempo","_solMax","_nul","_coste","_tipo","_threatEvalAir","_threatEvalLand","_pos","_timeOut","_plane"];
...
if (_CSAT) then
	...
	for "_i" from 1 to _cuenta do
		...
		if (not(_tipoveh in opHeliTrans)) then
			...
			_grupo = [_posorigen, side_red, (opCfgInf >> opISquad)] call BIS_Fnc_spawnGroup;
	...
	[_mrkDestino] spawn
		...
		for "_i" from 0 to round (random 2) do
			{
			_nul = [_mrkdestino,opCASFW] spawn airstrike;
			sleep 30;
			};
...
if (_aeropuerto != "") then
	...
	for "_i" from 1 to 3 do
		...
		if (_i == 3) then
			...
			if ((_threatEvalAir > 10) && (count (_planesAAF - heli_armed) < count _planesAAF)) then {_planesAAF = _planesAAF - heli_unarmed};
			if ((_threatEvalAir > 15) && (count (_planesAAF - planes) < count _planesAAF)) then {_planesAAF = planes};
