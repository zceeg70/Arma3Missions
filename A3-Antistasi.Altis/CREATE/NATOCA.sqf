...
for "_i" from 1 to _cuenta do
	...
	*/
	{_x setBehaviour "CARELESS";} forEach units _grupoheli;
	_nul = [_heli,"NATO Air Transport"] spawn inmuneConvoy;
	if (_tipoveh == "B_Heli_Transport_01_camo_F") then
		{
		_grupo = [_orig, side_blue, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad_Weapons")] call BIS_Fnc_spawnGroup;
	...
	if (_tipoveh == "B_Heli_Light_01_F") then
		{
		{_x disableAI "TARGET"; _x disableAI "AUTOTARGET"} foreach units _grupoheli;
		_grupo = [_orig, side_blue, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfTeam")] call BIS_Fnc_spawnGroup;
		{_x assignAsCargo _heli; _x moveInCargo _heli; _soldados = _soldados + [_x]; [_x] spawn NATOinitCA} forEach units _grupo;
		_grupos = _grupos + [_grupo];
		_landpos = [];
		_landpos = [_posicion, 0, 500, 10, 0, 0.3, 0] call BIS_Fnc_findSafePos;
	...
	if (_tipoveh == "B_Heli_Transport_03_F") then
		{
		{_x disableAI "TARGET"; _x disableAI "AUTOTARGET"} foreach units _grupoheli;
		_grupo = [_orig, side_blue, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfAssault")] call BIS_Fnc_spawnGroup;
		{_x assignAsCargo _heli; _x moveInCargo _heli; _soldados = _soldados + [_x]; [_x] spawn NATOinitCA} forEach units _grupo;
		_grupos = _grupos + [_grupo];
