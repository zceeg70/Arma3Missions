...
switch _tipo do
	...
	case "B_G_Soldier_M_F":
		...
		if (("srifle_GM6_F" in unlockedWeapons) or (hayRHS)) then
			...
			_unit removeWeaponGlobal (primaryWeapon _unit);
			if ("srifle_GM6_F" in unlockedWeapons) then {[_unit, "srifle_GM6_SOS_F", 8, 0] call BIS_fnc_addWeapon}
			else {[_unit, "rhs_weap_svdp_wd", 8, 0] call BIS_fnc_addWeapon; _unit addPrimaryWeaponItem "rhs_acc_pso1m2";}
			};
		_skillSet = 6;
		};
