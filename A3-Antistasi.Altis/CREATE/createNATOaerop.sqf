...
if (count _buildings > 1) then
	...
	while {(spawner getVariable _marcador) and (_cuenta < round (5*_prestigio))} do
		{
		_tipoveh = (planesNATO - ["B_Plane_CAS_01_F"]) call BIS_fnc_selectRandom;
		...
		_unit = _grupo createUnit ["B_Pilot_F", _posicion, [], 0, "NONE"];
		_cuenta = _cuenta + 1;
		};
...
_veh = createVehicle ["Flag_NATO_F", _pos, [],0, "CAN_COLLIDE"];
...
_grupo = [_posicion, side_blue, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad")] call BIS_Fnc_spawnGroup;
sleep 1;
...
while {(spawner getVariable _marcador) and (_cuenta < _nVeh)} do
	...
	if (diag_fps > minimoFPS) then
		...
		_grupo = [_pos,side_blue, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad")] call BIS_Fnc_spawnGroup;
		sleep 1;
